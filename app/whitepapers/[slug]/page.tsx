...
export async function generateStaticParams() {
  const posts = getMdxFiles("whitepapers");
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const post = getMdxBySlug("whitepapers", slug);
  if (!post) return {};
  
  return {
    title: `${post.title} | Whitepapers`,
    description: post.description,
  };
}

export default async function WhitepaperPostPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const post = getMdxBySlug("whitepapers", slug);
        ...
        <div className="max-w-screen-xl mx-auto">
          ...
          <div className="max-w-5xl mx-auto flex flex-col lg:flex-row gap-16">
            ...
            <aside className="w-full lg:w-64 shrink-0 space-y-12 border-t border-zinc-200 pt-12 lg:border-t-0 lg:pt-0">
              <div>
                <h4 className="font-mono text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-6">Author</h4>
                <AuthorCard authorName={post.author} date={post.publishedAt} />
              </div>
              <ShareButtons title={post.title} slug={post.slug} type="whitepapers" />
            </aside>
          </div>
          ...
          {relatedPosts.length > 0 && (
            <div className="max-w-5xl mx-auto mt-32 pt-16 border-t border-zinc-200">
              <h3 className="text-2xl font-serif font-bold text-zinc-950 mb-8">Related Papers</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {relatedPosts.map(rp => (
                  <Link key={rp.slug} href={`/whitepapers/${rp.slug}`} className="group block p-6 bg-white rounded-2xl border border-zinc-200 hover:border-blue-500 hover:shadow-xl transition-all">
