...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    title: `${post.title} | Rishvin Labs Blog`,
        ...
        <div className="max-w-screen-xl mx-auto">
          ...
          {relatedPosts.length > 0 && (
            <div className="max-w-5xl mx-auto mt-32 pt-16 border-t border-zinc-200">
              <h3 className="text-2xl font-serif font-bold text-zinc-950 mb-8">Keep Reading</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {relatedPosts.map(rp => (
                  <Link key={rp.slug} href={`/blog/${rp.slug}`} className="group block p-6 bg-white rounded-2xl border border-zinc-200 hover:border-blue-500 hover:shadow-xl transition-all">
